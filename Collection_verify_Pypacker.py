...
from pypacker.layer4 import tcp #added
        ...
        try:
            while 1:
                ...
                for res in result:
                    ...
                    Received=format(ethRaw.SequenceCnt)
                   
                    #print(ethRaw)                    
                    #nprint('Received: {0}'.format(ethRaw.SequenceCnt))
                                  
                    eth = ethernet.Ethernet(ethRaw.EthFullFrame)
                    #nprint('decode Eth: src:{0} dst: {1}'.format(eth.src_s, eth.dst_s))

                    #nprint('Decoder: {0}'.format(eth))
                    #Dict
                    dict1 =  {'Received data': Received, 'Source address':eth.src_s, 'Destination address':eth.dst_s}
